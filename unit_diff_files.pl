# Name the unit test
$unit_test->name('unit_diff_files.pl');
my $input_file = 'input';
...
$unit_test->log("[TEST] Test pomoci diffu externich souboru");
...
#Nothing on stdin, first parameter for program is input, second one is output
$unit_test->run('/dev/null', $input_file, $output_file);
#OR with Valgrind:
#$unit_test->run_valgrind('/dev/null', $input_file, $output_file);
...
$unit_test->analyze_stdout('diff', $output_file, $expected_file);
